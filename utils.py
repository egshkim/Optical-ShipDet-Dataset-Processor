...
def tile_image_and_labels(
    ...
    """
    Tiles large images and adjusts label coordinates accordingly.
    
    Args:
        image: PIL Image to be tiled
        labels: List of labels in format [class_id, x1, y1, x2, y2, x3, y3, x4, y4]
    ...
    for ty in range(n_tiles_y):
        for tx in range(n_tiles_x):
            ...
            for label in labels:
                ...
                box_points = [(abs_coords[i], abs_coords[i+1]) for i in range(0, len(abs_coords), 2)]
...
def process_image_and_labels(
    ...
    if img_width > target_width or img_height > target_height:
        if tile_if_large:
            ...
            return random_pad_image_and_rel_labels(image, adjusted_labels, 
                                                 target_width, target_height)
    else:
        return random_pad_image_and_rel_labels(image, labels, 
                                             target_width, target_height)
...
def visualize_labels(
    images: Union[str, Image.Image, List[Tuple[str, Image.Image, List[List[float]]]]],
    ...
    Args:
        images: Single image, path, or list of (name, image, labels) tuples
        labels: Labels for single image case
        output_path: Path to save visualizations
        thickness: Line thickness
        num_samples: Number of random samples for batch processing
        class_colors: Optional pre-defined color list for classes
        num_classes: Number of classes to generate colors for if class_colors not provided
        random_seed: Random seed for reproducibility
    
    Returns:
        Visualized image(s) with colored bounding boxes
    """
    ...
    if isinstance(images, (str, Image.Image)):
        return _visualize_single_image(images, labels, output_path, class_colors, thickness)
    
    # Handle multiple images case
    if not isinstance(images, list):
        raise ValueError("images must be either a single image or a list of (name, image, labels) tuples")
    ...
    for idx, (name, image, img_labels) in enumerate(images):
        try:
            vis_img = _visualize_single_image(image, img_labels, None, class_colors, thickness)
            
            if output_path:
                if os.path.isdir(output_path):
                    save_path = os.path.join(output_path, f"{name}_vis.png")
                elif '{}' in output_path:
                    save_path = output_path.format(idx)
                else:
            ...
            logging.error(f"Error visualizing image {name}: {str(e)}")
            continue
